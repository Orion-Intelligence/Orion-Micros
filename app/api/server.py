...
async def run_with_cancellation(request: Request, func, timeout: int, default_result):
    stop_event = threading.Event()
    task = asyncio.create_task(asyncio.to_thread(func, stop_event))
    try:
        while not task.done():
...
class APIService:
    ...
    async def cti_classify(self, request: parse_cti_model, raw_request: Request):
        logger.info("Received request at /cti_classifier/classify")
        async with self.semaphore:
            func = lambda stop_event: self.m_cti_parser.invoke_trigger(CTI_REQUEST_COMMANDS.S_PARSE, [request.data], stop_event=stop_event)
            try:
                result = await run_with_cancellation(raw_request, func, timeout=15, default_result={"result": {"label": "none"}})
                return result
            except Exception:
    ...
    async def runtime_parse(self, request: Request):
        try:
            ...
            response = await asyncio.wait_for(self.m_runtime_parser.get_email_username(query), timeout=120)
            return response
        except asyncio.TimeoutError:
    ...
    async def process_request(self, request_data, raw_request: Request, command, controller, default_result, timeout=60):
        async with self.semaphore:
            func = lambda stop_event: controller(command, request_data, stop_event=stop_event)
            try:
                result = await run_with_cancellation(raw_request, func, timeout=timeout, default_result={"result": default_result})
                return result
            except Exception:
    ...
    async def nlp_parse(self, request: parse_request_model, raw_request: Request):
    ...
    async def nlp_parse_ai(self, request: parse_request_model, raw_request: Request):
    ...
    async def nlp_summarise_ai(self, request: parse_request_model, raw_request: Request):
    ...
    async def topic_classifier_predict(self, request: classify_request_model, raw_request: Request):
        logger.info("Received request at /topic_classifier/predict")
        return await self.process_request(
            request_data=[request.title, request.description, request.keyword],
            raw_request=raw_request,
    ...
    async def ocr_parse(self, file: UploadFile = File(...), request: Request = None):
        ...
        async with self.semaphore:
            func = lambda stop_event: self.ocr_controller_instance.invoke_trigger(OCR_REQUEST_COMMANDS.S_PARSE, [content], stop_event=stop_event)
            try:
                result = await run_with_cancellation(request, func, timeout=60, default_result={"text": ""})
                return {"text": result}
            except asyncio.TimeoutError:
